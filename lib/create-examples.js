...
var imageSaver = require('../lib/ImageSaver');
...
var CreateExamples = function () {
 ...
 "<!DOCTYPE html>" +
    ...
    function getPagesContentNodesRecursive(accessToken, pages, pagesMetadata, index, callback) {
        ...
        return onenoteApi.getPageJsonContent(accessToken, function (error, httpResponse, body) {
            ...
            var pageMetadata =
            {
                ...
                parentSection: page.parentSection.name,
                parentNotebook: page.parentNotebook.name
            };
    ...
    this.getLastDayPageMetadata = function (accessToken, callback) {
        ...
        }, { orderBy: "lastModifiedTime desc", filter: filter, expand: "parentNotebook,parentSection", top: "50" });
    };
    ...
    this.getTermMetadata = function (accessToken, callback, terms) {
        this.getLastDayPageMetadata(accessToken, function (error, pagesMetadata) {
	        ...
	        return imageSaver.saveImagesFromTermMetadata(accessToken, termMetadata, function(error) {
		        if (error) {
                    console.log("Error when saving image metadata.");
			        console.log(error);
		        }
    ...
    this.saveAnImage = function (accessToken, callback) {
        var content = fs.readFileSync(path.normalize(__dirname + "/../public/images/doge.jpg"));
	    var imageUrl = imageSaver.saveImage(content, "myfavoriteImage");
	    return callback(null, {imageUrl: imageUrl});
    };
};
