...
var CreateExamples = function() {
	...
	function getPagesContentNodesRecursive(accessToken, pages, pagesMetadata, index, callback) {
		...
		return onenoteApi.getPageJsonContent(accessToken, function(error, httpResponse, body) {
			...
			// Get paragraph nodes
			var paragraphMetadata = oneNoteNodes.getParagraphNodeMetadata(parsedBody, page);
	...
	this.getLastDayPageMetadata = function (accessToken, callback) {
		...
		var filter = "lastModifiedTime ge " + dateString;
		...
		}, { filter: "parentNotebook/isShared eq true", orderBy:"lastModifiedTime desc", filter: filter });
	};
	
	this.getTermMetadata = function (accessToken, callback) {
		this.getLastDayPageMetadata(accessToken, function(error, pagesMetadata) {
			if (error) {
				return callback(error);
			}

			var termMetadata = metadataFilters.getTermsMetadata(pagesMetadata, ["hackaton", "disney"]);
