...
var LiveConnectClient = function () {
	 ...
	 * @returns {string} The Live Api endpoint URL
	 */
	this.getLivePicApiUrl = function (accesstoken) {
		var query = toQueryString({
			'access_token': accesstoken
		});
	...
	this.getUserInfo = function (accessToken, req, callback) {
		...
		request.get(options,
			...
			if (error) {
				...
				callback(null, req, response, body);
			}
		});
	};

	this.getUserPic = function(accessToken, callback) {
		var options = {
            url: this.getLivePicApiUrl(accessToken),
            method: 'HEAD'
        };
		request.get(options,
			function (error, response, body) {
